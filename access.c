...
#if DEBUG 
#define ACCDEBUG(xx) printf xx
#else
#define ACCDEBUG(xx) do { ; } while(0)
#endif
					...
					unsigned *retidxblk, unsigned wrtflg)
{
	...
	if ( wrtflg && ((vcb->status & VCB_WRITE) == 0) )
		return SS$_WRITLCK;
	...
	if ( vcbdev->idxfcb->head != NULL ) {
		if ( idxblk >= VMSSWAP(vcbdev->idxfcb->head->fh2$w_recattr.fat$l_efblk) ) {
			printf("accesshead(): Not in index file\n");
...
{
	register struct WCB *wcb = (struct WCB *)calloc(1, sizeof(struct WCB));
	if ( wcb == NULL ) {
		*retsts = SS$_INSFMEM;
	} else {
		unsigned curvbn;
		...
		if ( wcbkey->prevwcb == NULL ) {
			...
			ACCDEBUG(("wcb_create(): Found wcbkey->prevwcb==NULL. before premap_indexf: head=%p\n",(void *)head));
			if ( head == NULL ) {
			...
			ACCDEBUG(("wcb_create(): Found wcbkey->prevwcb==NULL After premap_indexf: head=%p.\n",(void *)head));
			...
			ACCDEBUG(("wcb_create(): Found wcbkey->prevwcb != NULL. wcb->hd_seg_num=%d, vioc=%p, head=%p\n", wcb->hd_seg_num, (void *)vioc, (void *)head));
		}
		do {
			...
			if ( wcb->hd_seg_num != 0 ) {
				*retsts = accesshead(wcbkey->fcb->vcb, &wcb->hd_fid, wcb->hd_seg_num, &vioc, &head, NULL, 0);
				...
				ACCDEBUG(("wcb_create(): After accesshead() call. wcb->hd_seg_num=%d, vioc=%p, head=%p\n", wcb->hd_seg_num, (void *)vioc, (void *)head));
			} else {
				if ( head == NULL ) {
					head = premap_indexf(wcbkey->fcb, retsts);
					if ( head == NULL )
				...
				ACCDEBUG(("wcb_create(): Found wcb->hd_seg_num=0, head=%p\n", (void *)head));
			}
...
#if DEBUG
		{
			...
			unsigned long tot=0;
			char msg[512];
			len = snprintf(msg,sizeof(msg),"wcb_create(): Found %d extents: ", extents );
			for (ii=0; ii < extents;++ii)
			{
				tot += wcb->phylen[ii]*512;
				len += snprintf(msg + len, sizeof(msg) - len, "%s0x%X/%d", ii == 0 ? "" : ",", wcb->phyblk[ii],wcb->phylen[ii]);
			}
			printf("%s\n\tTotal byte count: %lu\n",msg,tot);
		}
#endif
				   ...
				   unsigned *hdrseq)
{
	...
	ACCDEBUG(("Accessing window for vbn %d(0x%X), file (%x)\n", vbn, vbn, fcb->cache.hashval));
	wcbkey.vbn = vbn;
	wcbkey.fcb = fcb;
	wcbkey.prevwcb = NULL;
		...
		return sts;
	{
		...
		*phylen = wcb->phylen[extent] - togo;
		...
		ACCDEBUG(("Mapping vbn %d to %d (%d -> %d)[%d] file (%x)\n",
...
{
	register struct VIOC *vioc = (struct VIOC *)calloc(1, sizeof(struct VIOC));
	if ( vioc == NULL ) {
		*retsts = SS$_INSFMEM;
	} else {
		register int length;
		...
		do {
			if ( fcb->highwater != 0 && curvbn >= fcb->highwater ) {
				...
				if ( sts & 1 ) {
					...
					ACCDEBUG(("vioc_create(): Asking to read 0x%X bytes into vioc->data[%ld]\n",phylen*512, (address-(char *)vioc->data)/512));
					if ( address+phylen*512 > (char *)vioc->data+(512*VIOC_CHUNKSIZE)) {
						int newBlocks = ((char *)vioc->data+(512*VIOC_CHUNKSIZE)-address)/512;
						printf("vioc_create(): ERROR! Attempting to read too much data. Trimming it back from %d blocks to %d\n",phylen,newBlocks);
						phylen = newBlocks;
					}
					 ...
					 char **retbuff, unsigned *retblocks, unsigned wrtblks)
{
	...
	ACCDEBUG(("accesschunk(): At entry: vbn:0x%X fcb->hiblock:0x%X, blocks=0x%X, wrtblks:%u\n", vbn, fcb->hiblock, blocks, wrtblks));
	if ( wrtblks ) {
		if ( (fcb->status & FCB_WRITE) == 0 )
			return SS$_WRITLCK;
	...
	*retbuff = vioc->data[blocks];
	ACCDEBUG(("accesschunk(): At entry: vbn:0x%X fcb->hiblock:0x%X, blocks:0x%X\n", vbn, fcb->hiblock, blocks));
...
{
	ACCDEBUG(("Deaccessing file (%x) reference %d\n", fcb->cache.hashval, fcb->cache.refcount));
	if ( fcb->cache.refcount == 1 ) {
		...
		if ( refcount != 0 ) {
			printf("deaccessfile(): File reference counts non-zero %d  (%d)\n", refcount, fcb->cache.hashval);
			ACCDEBUG(("deaccessfile(): File reference counts non-zero %d %d\n",
...
{
	register struct FCB *fcb = (struct FCB *)calloc(1, sizeof(struct FCB));
	if ( fcb == NULL ) {
		*retsts = SS$_INSFMEM;
	} else {
